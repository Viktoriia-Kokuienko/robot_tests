...
Можливість проведення 1 го раунду аукціону для другого учасника
  ...
  ...      auction
  Вибрати учасника, який може зробити ставку
  Поставити ставку меншу від мінімальної на 1 грн
...
Можливість проведення 2 го раунду аукціону для першого учасника
  ...
  ...      auction
  Вибрати учасника, який може зробити ставку
 # Поставити велику ставку в 1 грн
...
Відкрити сторінку аукціону для ${username}
  ...
  ...      Click Element                  xpath=//button[contains(@class, 'btn btn-success btn-lg btn-block ng-scope')]
...
Дочекатись завершення паузи перед першим раундом для користувачів
  ...
  # ${pause_end}=   Wait Until Keyword Succeeds
  # ...      30 times
  # ...      5s
  # ...      Get Text  id=round_information_block
  # Log  ${pause_end}  WARN
  # Should Be True  ${pause_end} == '→ 1'
  ${date}=  Get Current Date
...
Поставити мінімально можливу ставку
  Wait Until Page Contains Element    id=min_bid_amount_price
  ${last_amount}=     Get Text        id=min_bid_amount_price
  ${last_amount}=  convert_amount_string_to_float  ${last_amount}
  ${value}=  Convert To Number  0.01
  ${last_amount}=  subtraction  ${last_amount}  ${value}
  Поставити ставку   ${last_amount}   Заявку прийнято
...
Поставити ставку меншу від мінімальної на ${low_amount} грн
  Wait Until Page Contains Element  id=min_bid_amount_price
  ${last_amount}=  Get Text         id=min_bid_amount_price
  ${last_amount}=  convert_amount_string_to_float  ${last_amount}
  ${low_amount}=  convert_amount_string_to_float  ${low_amount}
  ${last_amount}=  Evaluate  ${last_amount}-${low_amount}
...
#Поставити велику ставку в ${last_amount} грн
#  Wait Until Page Contains Element  id=min_bid_amount_price
#  Поставити ставку  ${last_amount}  Ви ввели дуже велику суму
